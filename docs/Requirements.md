...
The configuration framework shall make it possible to use environment variables in the configuration.


## [R3.1] Initial environment variables

The application shall be able to load the process's environment into the environment variables object. It shall also be able to load and application specific environment variables.
...
A reference to an environment variable shall be in the following format: ```${environment_variable_name}```.
...
### [R4.5] Referencing of another configuration parameter node
...
All *JSON values* that are not *JSON Objects* and not reference nodes are interpreted as a Value node. To explicitly interpret a *JSON Object* as a Value node then the parameter's name needs to be prefixed with the *explicit value* decorator ('#' character).

The *explicit value* decorator shall be allowed also for other *JSON value* types, but it shall have no effect on them.
...
All *JSON values* that are not *JSON Objects* and not reference nodes are interpreted as a Value node. To explicitly interpret a *JSON Object* as a Value node AND resolve all references to environment variables within it then the parameter's name needs to be prefixed with the *explicit value with references to environment variables* decorator ('&' character).

The references to environment variables shall be allowed in *JSON Object* names and *JSON values* of String type anywhere in the hierarchy (including in *JSON Arrays*).

The *explicit value with references to environment variables* decorator shall be allowed also for other *JSON value* types, but it shall have no effect on them.
...
For constitency these additional members ("file_path", "source_node", "destination_node") shall also be used in other configuration file types (if applicable).
...
2. If present the configuration files from its *includes* member shall be individually loaded and their references resolved (before reading the next included configuration file)
3. The configuration parameters from the *config* member shall be loaded and appliead over the currently read configuration (from included configuration filed)
4. All the references from the resulting configuration shall be resolved.
5. The final configuration parameter structure shall be generated from the loaded configuration files. This structure shall have the same data types and rules as *JSON*.
...
* Configuration container: a sequential or associative container holding items of the same type of *configuration object*

Each *configuration parameter* and *configuration object* hall provide the following information:
...
It shall be possible to store an Object node to a file (of *CppConfigFramework* type). The Object node's structure shall be written to the *config* member (no *environment_variables* or *includes* members).
